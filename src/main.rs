...
use serenity::all::{CreateInteractionResponse, CreateInteractionResponseMessage, Interaction, UserId};
...
impl EventHandler for Handler {
	async fn message(&self, ctx: Context, msg: Message) {
		...
		if let Err(err) = commands::typ_message(&ctx, content, &msg).await {
			msg.reply(&ctx, format!(":no_entry_sign: Error: {err}")).await.unwrap();
		}
	}
		...
		match interaction {
			Interaction::Command(cmd) => {
				...
				if let Err(err) = commands::typ_interaction(&ctx, &cmd).await {
					let msg = CreateInteractionResponseMessage::new().content(format!(":no_entry_sign: Error: {err}"));
